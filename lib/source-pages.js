...
import { sourcePageMediaReplacements } from "./site-images";
...
function decorateMain(routeKey, main) {
  const replacements = sourcePageMediaReplacements[routeKey];

  if (!replacements?.length) {
    return main;
  }

  return replacements.reduce((content, [from, to]) => content.replaceAll(from, to), main);
}
...
export async function getSourcePage(routeKey = "") {
  ...
  return {
    ...
    main: decorateMain(routeKey, extractMatch(html, /<main[^>]*>([\s\S]*?)<\/main>/i))
  };
}
