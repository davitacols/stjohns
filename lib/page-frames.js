import { featureStripImagesByRoute } from "./site-images";
...
function attachCardImages(routeKey, cards) {
  return cards.map((card, index) => {
    const image = featureStripImagesByRoute[routeKey]?.[index];
...
export const routeFeatureStrips = Object.fromEntries(
  Object.entries(baseRouteFeatureStrips).map(([routeKey, strip]) => [
    routeKey,
    {
      ...strip,
      cards: attachCardImages(routeKey, strip.cards)
    }
  ])
);
